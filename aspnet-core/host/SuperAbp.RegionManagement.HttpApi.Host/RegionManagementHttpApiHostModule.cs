...
using SuperAbp.RegionManagement.MultiTenancy;
...
public class RegionManagementHttpApiHostModule : AbpModule
{
    ...
    public override void ConfigureServices(ServiceConfigurationContext context)
    {
        ...
        if (hostingEnvironment.IsDevelopment())
        {
            Configure<AbpVirtualFileSystemOptions>(options =>
            {
                ...
                options.FileSets.ReplaceEmbeddedByPhysical<RegionManagementDomainModule>(Path.Combine(hostingEnvironment.ContentRootPath, string.Format("..{0}..{0}src{0}SuperAbp.RegionManagement.Domain", Path.DirectorySeparatorChar)));
                ...
                options.FileSets.ReplaceEmbeddedByPhysical<RegionManagementApplicationModule>(Path.Combine(hostingEnvironment.ContentRootPath, string.Format("..{0}..{0}src{0}SuperAbp.RegionManagement.Application", Path.DirectorySeparatorChar)));
            });
        }
        ...
        context.Services.AddAbpSwaggerGenWithOAuth(
            ...
            {
                {"RegionManagement", "RegionManagement API"}
            },
            options =>
            {
        ...
        context.Services.AddAuthentication(JwtBearerDefaults.AuthenticationScheme)
            .AddJwtBearer(options =>
            {
                ...
                options.Audience = "RegionManagement";
            });

        Configure<AbpDistributedCacheOptions>(options =>
        {
            options.KeyPrefix = "RegionManagement:";
        });

        var dataProtectionBuilder = context.Services.AddDataProtection().SetApplicationName("RegionManagement");
        if (!hostingEnvironment.IsDevelopment())
        {
            var redis = ConnectionMultiplexer.Connect(configuration["Redis:Configuration"]);
            dataProtectionBuilder.PersistKeysToStackExchangeRedis(redis, "RegionManagement-Protection-Keys");
        }
    ...
    public override void OnApplicationInitialization(ApplicationInitializationContext context)
    {
        ...
        app.UseAbpSwaggerUI(options =>
        {
            ...
            options.OAuthScopes("RegionManagement");
        });
        app.UseAuditing();
